...
// List all events with country data
router.get('/', function(req, res, next) {
  const events = Event.all
  res.render('events/index', { title: 'Imperial Footprints || Events', events: events });
});

// Show event with country data

router.get('/show/:id', async (req, res, next) => {
  let templateVars = {
    title: 'Imperial Footprints || Events', event: Event.get(req.params.id)
  }
  if (templateVars.event.countryId) {
    templateVars['country'] = Country.get(templateVars.event.countryId);
  }
